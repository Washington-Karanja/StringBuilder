...
import { consultationTypes } from "@/data/content";

export const Route = createFileRoute("/audit")({
  head: () => ({ meta: [{ title: "The Loyalty, CRM & Data Audit — Fikiri Communication" }, { name: "description", content: "An independent, benchmarked diagnostic for loyalty, CRM and customer data." }] }),
  component: Audit,
});

const steps = [
  ["Pre-engagement", "We send a focused data request and get access to what we need."],
  ["Assessment", "We score your maturity across each area against a structured framework."],
  ["Benchmarking", "We compare you against best-in-class and relevant competitor programmes."],
  ["Findings & roadmap", "You receive a clear report: where you stand, what it is costing you, and a prioritised plan for what to do next."],
];
...
function Audit() {
  return <div>
    ...
    <section className="mx-auto max-w-6xl px-5 py-20 sm:px-8"><Reveal><div className="rounded-3xl bg-primary px-8 py-14 text-center text-primary-foreground"><h2 className="text-3xl font-semibold">Request your audit</h2><p className="mx-auto mt-4 max-w-xl text-sm leading-relaxed text-primary-foreground/70">Individual audits start from 300,000. The three-pillar bundle starts from 750,000, and the audit fee is credited in full against follow-on implementation work.</p><Button asChild variant="hero" size="lg" className="mt-8"><Link to="/contact">Request an Audit</Link></Button></div></Reveal></section>
  </div>;
}
